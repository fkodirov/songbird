...
function newlvl(){
  ...
  if(document.querySelector('.lang').innerHTML=='EN')
  document.querySelector('.answers-info').innerHTML='<p style="padding: 10px; text-align: center;">Прослушайте аудиодорожку.</p><p style="padding: 10px; text-align: center;">Выберите птицу из списка.</p>';
  else
  document.querySelector('.answers-info').innerHTML='<p style="padding: 10px; text-align: center;">Listen to the audio track.</p><p style="padding: 10px; text-align: center;">Select a bird from the list.</p>';
...
nextlvl.addEventListener("click", function(){
  if(questionTitle.innerHTML=='**********' || questionTitle.innerHTML=='') return false;
    ...
    document.querySelector('.victory-text').innerHTML=`You scored ${scoreAll} out of 30!`;
  }
  else{
...
document.querySelector('.lang').addEventListener("click", function(e){
  if(e.target.innerHTML=='EN'){
    ...
    document.querySelectorAll('.page')[0].innerHTML='Warm up';
    document.querySelectorAll('.page')[1].innerHTML='Passerines';
    document.querySelectorAll('.page')[2].innerHTML='Forest birds';
    document.querySelectorAll('.page')[3].innerHTML='Songbirds';
    document.querySelectorAll('.page')[4].innerHTML='Raptors';
    document.querySelectorAll('.page')[5].innerHTML='Seabirds';

    if(document.querySelector('.question-title').innerHTML!='**********')
      document.querySelector('.question-title').innerHTML=birds[lvl][0]['name'];

    document.querySelector('.next-lvl-inner').innerHTML='Next level';
    document.querySelector('.new-game-inner').innerHTML='Play again!';  

    for(let i=0;i<6;i++){
      document.querySelectorAll('.answers-item')[i].childNodes[1].textContent=birds[lvl][i]['name'];
    }
    if(document.querySelector('.answers-info').children[0].tagName=='P'){
      document.querySelector('.answers-info').children[0].innerHTML='Listen to the audio track.';
      document.querySelector('.answers-info').children[1].innerHTML='Select a bird from the list.';
    }
    else{
      document.querySelector('.info-title').innerHTML=birds[lvl][0]['name'];
      document.querySelector('.sub').innerHTML=birds[lvl][0]['species'];
      document.querySelector('.description').innerHTML=birds[lvl][0]['description'];
    }

    document.querySelector('.victory-title').innerHTML='Congratulations!';
    document.querySelector('.victory-text').innerHTML=`You scored ${scoreAll} out of 30!`;
    ...
    localStorage.setItem('lang', document.querySelector('.lang').innerHTML);
    document.querySelectorAll('.page')[0].innerHTML='Разминка';
    document.querySelectorAll('.page')[1].innerHTML='Воробьиные';
    document.querySelectorAll('.page')[2].innerHTML='Лесные птицы';
    document.querySelectorAll('.page')[3].innerHTML='Певчие птицы';
    document.querySelectorAll('.page')[4].innerHTML='Хищные птицы';
    document.querySelectorAll('.page')[5].innerHTML='Морские птицы';

    if(document.querySelector('.question-title').innerHTML!='**********')
      document.querySelector('.question-title').innerHTML=birds[lvl][0]['name'];

    document.querySelector('.next-lvl-inner').innerHTML='Следующий уровень';
    document.querySelector('.new-game-inner').innerHTML='Попробовать ещё раз!';   

    for(let i=0;i<6;i++){
      document.querySelectorAll('.answers-item')[i].childNodes[1].textContent=birds[lvl][i]['name'];
    }
    if(document.querySelector('.answers-info').children[0].tagName=='P'){
      document.querySelector('.answers-info').children[0].innerHTML='Прослушайте аудиодорожку.';
      document.querySelector('.answers-info').children[1].innerHTML='Выберите птицу из списка.';
    }
    else{
      document.querySelector('.info-title').innerHTML=birds[lvl][0]['name'];
      document.querySelector('.sub').innerHTML=birds[lvl][0]['species'];
    ...
    document.querySelector('.victory-title').innerHTML='Поздравляем!';
    document.querySelector('.victory-text').innerHTML=`Вы набрали ${scoreAll} из 30!`;
